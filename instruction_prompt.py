...
class Config:
    def __init__(self):
        self.__conf_path = "./conf.toml"
        with open(self.__conf_path, "r") as f:
            self.__configs = toml.load(f)

        self.__tea_type = TeaTypeConfig.model_validate(self.__configs["tea_type"])

    def __dump(self):
        with open(self.__conf_path, "w") as f:
            _ = toml.dump(self.__configs, f)

    @property
    def tea_type(self) -> TeaTypeConfig:
        return self.__tea_type

    @tea_type.setter
    def tea_type(self, new_tea_type: TeaTypeConfig):
        self.__tea_type = new_tea_type
        self.__configs["tea_type"]["tea1"] = new_tea_type.tea1
        self.__configs["tea_type"]["tea2"] = new_tea_type.tea2
        self.__configs["tea_type"]["tea3"] = new_tea_type.tea3
        self.__dump()
        self.__tea_type = TeaTypeConfig.model_validate(self.__configs["tea_type"])
...
def set_system_instruction():
    ...
    お客様との雑談を通じて、お客様の気分や状況を推測し、
    最適な紅茶（tea1:{instruction_config.tea_type.tea1}、tea2:{instruction_config.tea_type.tea2}、tea3:{instruction_config.tea_type.tea3}）を選び、
    ...
    **お客様に提供する紅茶が決まったら、statusを「{ConversationStatus.FINISHED.value}」に設定し、`tea_data`フィールドに選択した紅茶の種類、砂糖、ミルクの量を格納してください。**
    ...
    - `tea_data`: お客様に提供する紅茶の詳細。statusが`{ConversationStatus.FINISHED.value}`の場合のみ、以下のオブジェクトを格納。
      - `type`: 紅茶の種類（`{TeaType.TEA1.value}`:{instruction_config.tea_type.tea1}, `{TeaType.TEA2.value}`:{instruction_config.tea_type.tea2}, `{TeaType.TEA3.value}`:{instruction_config.tea_type.tea3}）
